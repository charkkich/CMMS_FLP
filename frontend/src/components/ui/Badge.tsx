...
type WorkOrderStatus = 'Open' | 'Assigned' | 'In Progress' | 'Waiting Spare Part' | 'Completed' | 'Closed' | 'Cancelled';
...
export const RequestStatusBadge: React.FC<{ status: RequestStatus }> = ({ status }) => (
  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${requestStatusColors[status] || 'bg-gray-100 text-gray-700'}`}>
    {status}
  </span>
);

export const WorkOrderStatusBadge: React.FC<{ status: WorkOrderStatus }> = ({ status }) => (
  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${workOrderStatusColors[status] || 'bg-gray-100 text-gray-700'}`}>
    {status}
  </span>
);
